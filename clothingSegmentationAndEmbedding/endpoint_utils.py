...
def display_response(response_json, image, class_filter={}, consolidate_same_classes=True, outline_only=False):
    if consolidate_same_classes:
        ...
        for class_name in image_annotations:
            ...
            display_mask_on_image(image, mask_points, class_name, mask_color=mask_color, outline_only=outline_only)
    else:
        for i in range(len(response_json['result']['class_names'])):
            class_name = response_json['result']['class_names'][i]
            if (len(class_filter) > 0):
